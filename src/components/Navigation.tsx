...
const Navigation = ({ onFilter, onLogin }: NavigationProps) => {
        ...
        <div className="flex items-center justify-between h-16">
              ...
              {NavbarMenu.map((item) => (
                <li key={item.id} className="relative group">
                  {item.children ? (
                    <>
                      <button
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-lg transition-colors ${scrolled
                      ...
                      {/* Dropdown */}
                      <ul
                        className={`absolute left-1/2 transform -translate-x-1/2 mt-1 w-44 bg-white shadow-lg rounded-md p-2 opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all duration-200 ${scrolled ? "shadow-md" : "shadow-xl"
                          }`}
                      >
          ...
          <div className="hidden md:flex">
            {onLogin ? (
              <button
                onClick={onLogin}
                className={`ml-6 px-5 py-2 rounded-full font-medium transition-colors duration-300 ${scrolled
          ...
          <div className="md:hidden">
            <button
              className={`rounded-md p-2 ${scrolled
                ...
                {item.children ? (
                  <>
                    <button
                      ...
                      className={`flex justify-between items-center w-full text-left py-3 ${scrolled
                          ? "text-gray-700"
                    ...
                    >
                      ...
                      <FiChevronDown
                        size={16}
                        className={`transform transition ${activeDropdown === item.id ? "rotate-180" : ""
                          }`}
                      />
                    </button>
                        ...
                        {item.children.map((child, index) => (
                          <li key={`${item.id}-${index}`}>
                            <button
                              ...
                              className={`block py-2 pl-3 rounded-md ${scrolled
                  ...
                  <Link
                    to={item.link}
                    className={`flex items-center gap-2 py-3 ${scrolled
                        ? "text-gray-700"
            ...
            <li className="pt-2">
              {onLogin ? (
                <button
                  ...
                  className={`block w-full text-center px-5 py-2 rounded-full font-medium transition-colors duration-300 ${scrolled
                      ? "bg-green-600 text-white hover:bg-green-700"
                      : "bg-white text-green-700 hover:bg-gray-100"
                    }`}
                >
